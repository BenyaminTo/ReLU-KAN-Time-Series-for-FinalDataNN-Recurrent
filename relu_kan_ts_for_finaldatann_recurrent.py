...
class RecurrentReLUKANTS(nn.Module):
    ...
    1) Split into a sequence with length seq_step
    2) Apply ReLUKANLayer to each step
    3) Pass through LSTM
    4) Map to pred_len outputs
    """
    def __init__(self,
                 context_len: int,
                 pred_len: int,
                 ...
                 kan_out_dim: int,
                 rnn_hidden: int,
                 seq_step: int = 10,  # Length of each time step inside the window
                 num_layers: int = 1,
                 bidirectional: bool = False):
        super().__init__()
        assert context_len % seq_step == 0, "context_len must be divisible by seq_step."
        self.context_len = context_len
        self.pred_len = pred_len
        ...
        self.kan = ReLUKANLayer(input_size=seq_step,
                                ...
                                is_train=False)  # Set True if you want it trainable
        ...
        # Final LSTM output -> pred_len values
    ...
    def forward(self, x):
        ...
        if x.dim() == 2:
            ...
            # If DataLoader returns shape (batch, context_len, 1)
            x = x.squeeze(-1)
            b, L = x.shape
        else:
        ...
        # Split the window into num_steps segments of length seq_step:
        # x: (b, context_len) -> (b, num_steps, seq_step)
        x_steps = x.view(b, self.num_steps, self.seq_step)  # (b, T, D_step)

        # Apply ReLUKAN on each step
        # First send everything at once across the batch dimension
...
kan_out_dim = H_Dims  # For example, 32
rnn_hidden = H_Dims   # You can set this larger if needed
seq_step = 10         # Make sure Context_Len is divisible by this value
